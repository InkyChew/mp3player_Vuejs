      ...
      #right.col.px-0.d-flex.flex-column
        .music.mx-4.mt-auto.mb-2.d-flex.justify-content-between
          #info.col.px-0
            ...
            label Ed Sheeran
          #control.col.px-0.my-auto
            .row.mx-auto.d-flex.justify-content-between
              i.fas.fa-random.mx-3.my-auto(
                v-if="!isRadom"
                @click="random"
              )
              i.fas.fa-random.mx-3.my-auto.active(
                v-if="isRadom"
                @click="random"
              )
              i.fas.fa-step-backward.mx-2.my-auto(@click="prev")
              i.playNpause.fas.fa-pause-circle.fa-2x.mx-2.my-auto(
                v-if="isPlay"
                @click="pause"
              )
              i.playNpause.fas.fa-play-circle.fa-2x.mx-2.my-auto(
                v-if="!isPlay"
                @click="play"
              )
              i.fas.fa-step-forward.mx-2.my-auto(@click="next")
              i.fas.fa-retweet.mx-3.my-auto(
                v-if="!isRedo"
                @click="redo"
              )
              i.fas.fa-retweet.mx-3.my-auto.active(
                v-if="isRedo"
                @click="redo"
              )
          #vol.col.px-0.d-flex.justify-content-end
...
export default {
  ...
  methods: {
    initData () {
      const getNowMusic = this.getNowMusic
      this.nowMusic = getNowMusic
      ...
      if (this.isRadom) {
        ...
        console.log("random" + this.nowMusic)
      } else {
        this.next()
      }
    },
